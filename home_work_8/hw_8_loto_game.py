...
class Cask:
    ...
    def f(self):
        my_list = [x for x in range(1, self.amount + 1)]
        random.shuffle(my_list)
        for i, y in enumerate(my_list):
            print('{:*^30}'.format('*'))
            print(f'New cask: {y}, ({self.amount - (i + 1)} casks left)')
            yield y
...
def main():
    game = Loto(2)
    cask = Cask(90)
    player1 = Player('Your cart')
    player2 = Player('Computers cart')
    ...
    while True:
        ...
        inp_user = input('Strike out the number? (y/n)')
        if inp_user == 'y':
